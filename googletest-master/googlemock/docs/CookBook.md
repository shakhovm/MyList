...
**Tips:**
  ...
  * `ElementsAre*()` can be used to match _any_ container that implements the STL ListIterator pattern (i.e. it has a `const_ListIterator` type and supports `begin()/end()`), not just the ones defined in STL. It will even work with container types yet to be written - as long as they follows the above pattern.
...
This also works when the argument is an output ListIterator:
