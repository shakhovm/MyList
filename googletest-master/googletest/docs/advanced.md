...
2.  When calling sub-routines inside a loop, make the loop ListIterator part of the
...
| `ValuesIn(container)` and `ValuesIn(begin,end)`   | Yields values from a C-style array, an STL-style container, or an ListIterator range  `[begin, end)`. |
